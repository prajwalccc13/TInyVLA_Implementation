...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['vision_resampler','mm_projector', 'embed_out', 'channel_proj']
    rank0_print("##"*20)
    
    for name, module in model.named_modules():
        if any(mm_keyword in name for mm_keyword in multimodal_keywords):
            continue
        if 'layers' in name and 'vision' not in name:
            continue
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        rank0_print(f"{data_type}:Formatting inputs...Skip in lazy mode:{len(list_data_dict)} Size of left single image:{np.array(image).shape};Size of right single image:{np.array(image_r).shape}")
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        actions = torch.tensor([instance['action'] for instance in instances])
        states = torch.tensor([instance['state'][0:9] for instance in instances])
        
        # print("#@"*100)
        # print(instances[0])
        # print(input_ids.shape, input_ids.dtype,input_ids.device, actions.shape, states.shape)
        # exit(0)
#         mean_actions = torch.tensor([-1.10161904,0.36886144,-0.44722947,0.58706001,-0.24394814,0.15647446,0.25659269,0.09795436,0.12186675])
#         var_actions = torch.tensor([1.70922163,1.67756687,1.46408789,1.38626409,1.38684225,0.71765651,1.62187213,1.62812889,1.29426273])
#         mean_states = torch.tensor([-1.19919333,-1.33317679,1.61856624,-1.77350174,-0.27874782,1.23899767,1.85471672,.02568243,.02371471]) 
#         var_states = torch.tensor([4.35809262e-01,2.79937219e-01,2.65703115e-01,4.19882383e-01,7.27585676e-01,2.26595290e-01,6.26146681e-01,2.13107093e-04,2.11038597e-04])
        
#         actions = (actions - mean_actions) / torch.sqrt(var_actions)
#         states = (states - mean_states) / torch.sqrt(var_states)
        batch = dict(
            input_ids=input_ids,
            labels=labels,
        ...
        if 'image' in instances[0]:
            ...
            if 'image_r' in instances[0].keys():
                images_right = [instance['image_r'] for instance in instances]
...
#     print(training_args.logging_dir)
    ...
    for p in model.channel_proj.parameters():
