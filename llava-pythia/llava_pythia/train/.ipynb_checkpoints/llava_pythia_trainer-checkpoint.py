        ...
        if self.optimizer is None:
            ...
            if self.args.non_lora_lr is not None:
                ...
                for name, module in opt_model.named_parameters():
                    if 'layers' in name and 'vision' not in name: # gptneoxl LLM的参数
                        non_lora_parameters.append(name)

                    elif name in ['vision_resampler','mm_projector', 'embed_out', 'channel_proj']: # vision adapter、action head的参数
                        non_lora_parameters.append(name)
                    
                    if not isinstance(module, torch.nn.Linear): #非线性层
                        non_lora_parameters.append(name)
