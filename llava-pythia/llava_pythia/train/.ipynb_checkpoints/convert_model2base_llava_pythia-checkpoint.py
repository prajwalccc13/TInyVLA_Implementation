...
from transformers import CLIPVisionConfig, SiglipVisionConfig, SiglipImageProcessor, CLIPImageProcessor
...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        if "clip" in model_args.vision_tower:
            clip_or_siglip_config = CLIPVisionConfig.from_pretrained(model_args.vision_tower)
        elif "siglip" in model_args.vision_tower:
            clip_or_siglip_config = SiglipVisionConfig.from_pretrained(model_args.vision_tower)
        else:
            raise ValueError(f"Unsupported vision tower: {model_args.vision_tower}")
        vis_config = LlavaPythiaVisionConfig(**clip_or_siglip_config.to_dict())
    ...
    assert model_args.vision_tower is not None, "llava_phi only supports multi-modal models"
    if model_args.vision_tower is not None:
